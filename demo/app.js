...
function showHover(p, anchor) {
  ...
  hover.innerHTML = `
    ...
    <div class="quota"><div class="quota-top"><span>▦ Current window</span><span>Aug 20 – Aug 27</span></div></div>
    <div class="quota"><div class="quota-top"><span>〜 ${esc(p.primaryLabel)}</span><strong>${p.primaryPercent == null ? '—' : `${p.primaryPercent}% of 200K tokens`}</strong></div><div class="progress"><span style="width:${Math.min(p.primaryPercent ?? 0, 100)}%;background:${esc(p.accent)}"></span></div></div>
    <div class="quota"><div class="quota-top"><span>◷ Resets in</span><span>${esc(p.primaryReset || 'No reset')}</span></div>
    <div class="quota-top"><span>‹› Today's total</span><span>${esc(p.tokensToday || '—')} tokens</span></div>
    <div class="quota-top"><span>$ Estimated cost</span><span>${esc(p.costToday || '—')} USD</span></div>
...
function renderTray() {
  ...
  const rows = STATE.providers.slice(0, 6).map((p) => `
    ...
    <span>${p.primaryPercent ?? '—'}% of 200k</span></div>`).join('');
  el.innerHTML = `<div class="tray-head"><span class="brand-mark"></span><div><strong>UsageHalo</strong><div style="color:var(--muted);font-size:11px">All systems operational</div></div><span class="live-pill" style="margin-left:auto"><i></i>Live</span></div>${rows}<div class="tray-foot"><span>◈ Open Dashboard</span><span>⏻ Quit</span></div>`;
}
...
function renderSummaries() {
  ...
  const cards = [
    { label: 'Total Usage', value: '58%', meta: `${(tokens / 1e6 || 1.24).toFixed(2)}M / 2.15M tokens`, color: '#7c8aff' },
    { label: 'Total Cost', value: `$${cost ? cost.toFixed(2) : '12.47'}`, meta: '$3.21 over last week', color: '#b48aff' },
    { label: 'Requests', value: '1,429', meta: '+18% vs last week', color: '#5ec8d8' },
    { label: 'Active Providers', value: `${Math.min(6, STATE.providers.length)} / ${Math.min(6, STATE.providers.length)}`, meta: 'All connected', color: '#48d597' },
  ];
...
function renderHeatmap(id, metric) {
  ...
  for (let c = 0; c < 28; c++) for (let r = 0; r < 7; r++) {
    ...
    d.title = `${['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'][r]} · level ${d.dataset.l}`;
    el.appendChild(d);
  }
}
...
function renderModels() {
  const html = `<div class="model-row header"><span>Model</span><span>Provider</span><span>Usage</span><span>Cost</span><span>% of Total</span></div>` +
    MODEL_ROWS.map((r) => `<div class="model-row"><span class="model-name"><i class="model-provider-dot" style="background:${r[5]}"></i>${r[0]}</span><span>${r[1]}</span><span>${r[2]}</span><span>${r[3]}</span><span>${r[4]} <span class="pbar" style="display:inline-block;width:70px;height:5px;border-radius:99px;background:rgba(128,132,144,.2);vertical-align:middle"><span style="display:block;height:100%;width:${r[4]};background:${r[5]};border-radius:inherit"></span></span></span></div>`).join('');
  const a = $('#model-table'); if (a) a.innerHTML = html;
  const b = $('#model-table-full'); if (b) b.innerHTML = html + `<p class="muted small">Live instrumented rows appear here when opt-in request instrumentation posts to <code>/api/ingest/event</code>. Billing owner ≠ model vendor is preserved.</p>`;
}
const ALERTS = [
...
function renderAlerts() {
  const html = ALERTS.map((a) => `<div class="alert-item"><span>${a[0]}</span><span>${a[1]}</span><time>${a[2]}</time></div>`).join('');
  const a = $('#alert-list'); if (a) a.innerHTML = html;
  const b = $('#alert-list-full'); if (b) b.innerHTML = html;
  const c = $('#settings-alerts'); if (c) c.innerHTML = ['Claude usage at|73%', 'Gemini usage at|68%', 'Weekly budget at|62%', 'Codex usage reset in|2d 3h'].map((s) => { const [k, v] = s.split('|'); return `<div class="kv"><span>${k} <strong>${v}</strong></span><span style="display:flex;gap:8px;align-items:center"><select><option>Notify me</option><option>Silent</option></select><button class="toggle on"></button></span></div>`; }).join('');
  const d = $('#settings-budgets'); if (d) d.innerHTML = [['Monthly total budget', '$12.47 / $20.00', 62], ['Claude (monthly)', '$6.92 / $10.00', 69], ['OpenRouter (monthly)', '$3.21 / $5.00', 64]].map((r) => `<div style="padding:8px 0"><div class="kv"><span>${r[0]}</span><span>${r[1]} · ${r[2]}%</span></div><div class="progress"><span style="width:${r[2]}%"></span></div></div>`).join('');
  const e = $('#settings-sources'); if (e) e.innerHTML = [['UsageHalo Cloud', 'Connected'], ['Local Cache', 'Active'], [`Provider APIs`, `${Math.min(10, STATE.providers.length)} / 10 Connected`]].map((r) => `<div class="kv"><span>◉ ${r[0]}</span><span>${r[1]} ●</span></div>`).join('') + `<div class="kv"><span>View data source status</span><span>›</span></div>`;
}
function renderProviderPages() {
  ...
  if (bc) bc.innerHTML = [['Total budget', '$12.47 / $20.00', 62, 'Monthly'], ['Claude', '$6.92 / $10.00', 69, 'Subscription quota'], ['OpenRouter', '$3.21 / $5.00', 64, 'Credits']].map((r) => `<article class="panel"><div class="eyebrow">${r[3]}</div><h2>${r[0]}</h2><div class="budget-top"><strong>${r[1]}</strong><span>${r[2]}%</span></div><div class="progress xl"><span style="width:${r[2]}%"></span></div></article>`).join('');
}
